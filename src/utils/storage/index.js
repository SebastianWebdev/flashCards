...
const getSettings = async () => {
  try {
    ...
    console.log(
      'Wystąpił błąd przy próbie pobrania danych z lokalnej bazy danych Settings',
      error,
    );
  }
};
const saveSettings = async settings => {
  try {
    ...
    console.log(
      'Błąd przy próbie zapisu ustawień do pamięci urzadzenia',
      error,
    );
  }
};
const getCourses = async () => {
  try {
    ...
    console.log(
      'Wystąpił błąd przy próbie pobrania danych z lokalnej bazy danych Kursy',
      error,
    );
  }
};
...
const saveSingleCardToStorage = async card => {
  try {
    ...
    const cardIndex =
      coursIndex > -1 ? findItemByID(ID, courses[coursIndex].cards) : -1;
    if (cardIndex > -1) {
    ...
    console.log(
      'Błąd podczas próby zapisu pojedyńczego kursu do pamięci urządzenia',
      error,
    );
  }
};
